...
| Agent | Model | Job |
|---|---|---|
| `orchestrator` (root) | `groq/llama-3.3-70b-versatile` | Receives the problem, coordinates the team, compiles the final answer |
| `planner` | `groq/llama-3.3-70b-versatile` | Decomposes the problem into steps + edge cases + complexity, saves to state |
| `researcher` | `groq/llama-3.3-70b-versatile` | Web research via Exa for non-obvious algorithms |
| `solver` | `openai/gpt-4o` | Writes solution + tests, executes them in a sandbox, iterates until green |
| `critic` | `groq/llama-3.3-70b-versatile` | Checklist review; FAIL verdicts bounce back to the solver (max 3 attempts) |
